...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const allowed = ["gate_id","name","code","ip_address","port","rtsp_url","direction","status","protocol","confidence_threshold"];
    const sets: string[] = [];
    const vals: any[] = [];
    let idx = 1;
    ...
    vals.push(id);
    const result = await pool.query(
      `UPDATE parking_cameras SET ${sets.join(", ")} WHERE id = $${idx} RETURNING *`, vals
    );
    if (result.rows.length === 0) return NextResponse.json({ error: "Camera not found" }, { status: 404 });
    return NextResponse.json(result.rows[0]);
  } catch (err: any) {
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  try {
    const result = await pool.query("DELETE FROM parking_cameras WHERE id = $1 RETURNING id", [id]);
    if (result.rows.length === 0) return NextResponse.json({ error: "Camera not found" }, { status: 404 });

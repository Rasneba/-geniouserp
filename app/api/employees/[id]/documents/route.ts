...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM employee_documents WHERE employee_id = $1 ORDER BY id",
      [id]
    );
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { document_type, document_name, file_path, expiry_date } = await req.json();

  try {
    const result = await pool.query(
      `INSERT INTO employee_documents (employee_id, document_type, document_name, file_path, expiry_date)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [id, document_type, document_name, file_path, expiry_date]
    );
